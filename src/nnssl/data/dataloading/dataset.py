...
class nnSSLDatasetBlosc2(nnSSLBaseDataset):
    ...
    def __getitem__(self, image_identifier):
        ...
        #     try:
        #         data, anon, anat, properties = self.load_case(
        #             self.dataset_dir, self.image_dataset, image_identifier
        #         )

        #        # Convert to torch for NaN/Inf check
        #         torch_data = torch.as_tensor(data[:])  # Avoid keeping memmap reference

        #        # Skip corrupted data with NaN or Inf
        #         if torch.isnan(torch_data).any() or torch.isinf(torch_data).any():
        #            print(f"[WARNING] NaN/Inf detected in {image_identifier}. Skipping.")
        #            image_identifier = choice(self.image_identifiers)
        #            continue

        #         return data, anon, anat, properties

        #     except (RuntimeError, FileNotFoundError, EOFError) as e:
        #         print(f"[WARNING] Skipping {image_identifier} due to error: {e}")
        #         image_identifier = choice(self.image_identifiers)

        # raise RuntimeError(f"Too many invalid attempts for {image_identifier}.")
