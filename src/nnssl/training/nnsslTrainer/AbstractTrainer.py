...
class AbstractBaseTrainer(ABC):
    def __init__(
        ...
        self.is_ddp = dist.is_available() and dist.is_initialized()
        self.global_rank = dist.get_rank() if self.is_ddp else 0
        ...
        if self.is_ddp:
            print(
                f"I am global rank {self.global_rank}, local rank {self.local_rank}. {device_count()} GPUs are available. "
                f"The world size is {dist.get_world_size()}. Setting device to cuda:{self.local_rank}"
            )
            self.device = torch.device(type="cuda", index=self.local_rank)
        else:
            self.device = device
        ...
        self.momentum = 0.99
        self.nesterov = True
        self.num_iterations_per_epoch = 275
        self.num_val_iterations_per_epoch = 0
        self.num_epochs = 250
        self.current_epoch = 0
        self.total_batch_size = 120
    ...
    def get_tr_and_val_datasets(self):
        ...
        dataset_val = None #nnSSLDatasetBlosc2(
    ...
    def get_dataloaders(self):
        ...
        allowed_num_processes = 8 # get_allowed_n_proc_DA()
    ...
    def on_epoch_end(self):
       ...
       # self.print_to_log_file("val_loss", np.round(self.logger.my_fantastic_logging["val_losses"][-1], decimals=4))
        ...
        self.save_checkpoint(join(self.output_folder, "checkpoint_latest.pth"))
        if (self.current_epoch + 1) % 25 == 0 and self.local_rank == 0:
            self.print_to_log_file(f"Saving checkpoint at epoch {self.current_epoch + 1}...")
            self.save_checkpoint(
                join(self.output_folder, f"checkpoint_epoch_{self.current_epoch + 1}.pth")
            )
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        logging_dict = checkpoint["logging"]
        if "val_losses" in logging_dict and (not logging_dict["val_losses"]):
            print("⚠️ Skipping empty 'val_losses' from logger checkpoint.")
            del logging_dict["val_losses"]
    ...
    def do_split(self):
        ...
        if not isfile(splits_file):
            ...
            splits = {"train": list(train_subjects), "val": None} #list(val_subjects)}
